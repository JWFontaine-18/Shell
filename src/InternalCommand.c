#include "../include/InternalCommand.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>    
...
static int do_cd(tokenlist *tokens) {
    ...
    if (tokens->size > 2) { fprintf(stderr, "cd: too many arguments\n"); return 0; }
...
static void wait_all_background(backgroundProcs *procs) {
    // wait for any tracked background pids
    int status;
    for (int i = 0; i < 10; i++) {
        pid_t pid = procs->activeBackgroundProcessPIDs[i];
        if (pid > 0) {
            // block until finishes
...
bool is_internal_command(tokenlist *tokens) {
    if (!tokens || tokens->size == 0 || !tokens->items[0]) return false;
